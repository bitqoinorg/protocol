...
router.get("/qoin/balance", async (req, res) => {
  try {
    ...
    // ── Helper: fetch token accounts for one owner ──────────────────────────
    async function getTokenAccountsForOwner(ownerAddr: string): Promise<ParsedTokenAccount[]> {
      const [r1, r2] = await Promise.all([
        fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ jsonrpc: "2.0", id: "tok1", method: "getTokenAccountsByOwner",
            params: [ownerAddr, { programId: TOKEN_PROGRAM }, { encoding: "jsonParsed" }] }) })
          .then(r => r.json()) as Promise<{ result?: { value: ParsedTokenAccount[] } }>,
        fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ jsonrpc: "2.0", id: "tok2", method: "getTokenAccountsByOwner",
            params: [ownerAddr, { programId: TOKEN_2022_PROGRAM }, { encoding: "jsonParsed" }] }) })
          .then(r => r.json()) as Promise<{ result?: { value: ParsedTokenAccount[] } }>,
      ]);
      return [
        ...(r1.result?.value ?? []),
    ...
    // ── Fetch SOL balance + direct token accounts in parallel ────────────────
    const [directAccounts, solRes, accountInfoRes, sigsRes] = await Promise.all([
      getTokenAccountsForOwner(address),
      fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", id: "sol", method: "getBalance", params: [address] }) })
        .then(r => r.json()) as Promise<{ result?: { value: number } }>,
      // Check if this is an SPL multisig account
      fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", id: "acc", method: "getAccountInfo",
          params: [address, { encoding: "jsonParsed" }] }) })
        .then(r => r.json()) as Promise<{ result?: { value: { owner?: string; data?: { parsed?: { type?: string } } } | null } }>,
      // Get recent tx signatures to discover token mints
      fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jsonrpc: "2.0", id: "sigs", method: "getSignaturesForAddress",
          params: [address, { limit: 25 }] }) })
        .then(r => r.json()) as Promise<{ result?: Array<{ signature: string }> }>,
    ]);

    const solBalance = (solRes.result?.value ?? 0) / LAMPORTS_PER_SOL;

    // ── Detect SPL multisig: owner = Token Program + type = multisig ────────
    const TOKEN_PROGRAM_ID_STR = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA";
    const accountOwner = accountInfoRes.result?.value?.owner ?? "";
    const accountType = accountInfoRes.result?.value?.data?.parsed?.type ?? "";
    const isMultisig = accountOwner === TOKEN_PROGRAM_ID_STR && accountType === "multisig";

    let layer2Accounts: ParsedTokenAccount[] = [];

    if (isMultisig) {
      // ── Layer-2 discovery for SPL multisig Qoins ────────────────────────
      // Tokens live in ATAs owned by ATA(qoin, mint) rather than qoin directly.
      // Discover mints from: recent transactions + known popular tokens.
      const { getAssociatedTokenAddressSync } = await import("@solana/spl-token");
      const { PublicKey } = await import("@solana/web3.js");

      const sigList = (sigsRes.result ?? []).map(s => s.signature);
      const discoveredMints = new Set<string>(KNOWN_MINTS as unknown as string[]);

      // Parse up to 20 recent transactions to find mints
      if (sigList.length > 0) {
        try {
          const txBatch = await fetch(rpcUrl, { method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify(sigList.slice(0, 20).map((sig, i) => ({
              jsonrpc: "2.0", id: `tx${i}`, method: "getTransaction",
              params: [sig, { encoding: "jsonParsed", maxSupportedTransactionVersion: 0 }],
            }))) })
            .then(r => r.json()) as Array<{ result?: { meta?: { postTokenBalances?: Array<{ mint: string }> } } }>;

          for (const tx of (Array.isArray(txBatch) ? txBatch : [])) {
            for (const tb of (tx.result?.meta?.postTokenBalances ?? [])) {
              if (tb.mint) discoveredMints.add(tb.mint);
            }
          }
        } catch { /* best-effort */ }
      }

      // For each discovered mint, compute layer-2 owner and fetch its token accounts
      const qoinPk = new PublicKey(address);
      const layer2Queries = Array.from(discoveredMints).map(async (mint) => {
        try {
          const midAddr = getAssociatedTokenAddressSync(new PublicKey(mint), qoinPk, true).toBase58();
          return await getTokenAccountsForOwner(midAddr);
        } catch { return []; }
      });

      const layer2Results = await Promise.all(layer2Queries);
      layer2Accounts = layer2Results.flat();
    }

    // ── Merge direct + layer-2 accounts, deduplicate by mint (highest balance wins) ──
    const allCandidates = [...directAccounts, ...layer2Accounts];
    const mintBest = new Map<string, ParsedTokenAccount>();
    for (const acct of allCandidates) {
      const mint = acct.account.data.parsed.info.mint;
      const bal = acct.account.data.parsed.info.tokenAmount.uiAmount ?? 0;
      const existing = mintBest.get(mint);
      if (!existing || bal > (existing.account.data.parsed.info.tokenAmount.uiAmount ?? 0)) {
        mintBest.set(mint, acct);
      }
    }
    const allAccounts = Array.from(mintBest.values());
    ...
    const tokens = allAccounts.map((item) => {
      ...
      // authority = SPL token owner of this token account
      // If it equals the Qoin address (multisig), transfer works normally.
      // If it's some other address (a PDA intermediate), the token is locked.
      const authority: string = info.owner ?? "";
      const isLocked = authority !== address && authority !== "";
      return {
        mint: info.mint,
...
router.post("/qoin/init-all-slots", async (req, res) => {
  try {
    const { multisigAddress, mints: extraMints } = req.body as {
      multisigAddress: string;
      mints?: string[];
    };
    if (!multisigAddress) {
      res.status(400).json({ error: "multisigAddress required." });
      return;
    }

    const { getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, getAccount } = await import("@solana/spl-token");
    const connection = new Connection(getHeliusRpc(), "confirmed");
    const payer = getPlatformPayer();
    const multisigPubkey = new PublicKey(multisigAddress);

    // Combine known mints with any extra mints passed in
    ...
    // Check each mint's token program + whether its ATA already exists (in parallel)
    const existChecks = await Promise.all(
      allMints.map(async (mint) => {
        try {
          ...
          const tokenProgramId = mintInfo?.owner?.toBase58() === TOKEN_2022_STR
            ? TOKEN_2022_PROGRAM_ID
            : TOKEN_PROGRAM_ID;

          const ata = getAssociatedTokenAddressSync(mintPk, multisigPubkey, true, tokenProgramId);
          try {
            await getAccount(connection, ata, "confirmed", tokenProgramId);
            return { mint, ata: ata.toBase58(), tokenProgramId, exists: true };
          } catch {
            return { mint, ata: ata.toBase58(), tokenProgramId, exists: false };
          }
        } catch {
          return null;
        }
    ...
    const toCreate = existChecks.filter((c) => c && !c.exists) as Array<{ mint: string; ata: string; tokenProgramId: PublicKey }>;
    const skipped = existChecks.filter((c) => c && c.exists).map((c) => c!.mint);

    if (toCreate.length === 0) {
      res.json({ initialized: [], skipped, message: "All slots already initialized." });
      return;
    }

    // Batch into groups of 3 per TX (safe size, leaving room for different token programs)
    const BATCH = 3;
    ...
    for (let i = 0; i < toCreate.length; i += BATCH) {
      ...
      for (const { mint, ata, tokenProgramId } of batch) {
        tx.add(
          createAssociatedTokenAccountInstruction(
            payer.publicKey,
            new PublicKey(ata),
    ...
    res.json({ initialized, skipped, signatures });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : "Unknown error";
    res.status(500).json({ error: msg });
  }
